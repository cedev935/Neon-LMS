...
class LessonsController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->show_deleted == 1) {
            ...
            $lessons = Lesson::query()->where('live_lesson', '=', 0)->with('course')->onlyTrashed();
        }
        ...
        return DataTables::of($lessons)
            ...
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $request) {
                //$courseTimeLine = $q['courseTimeLine'];
                ...
                if ($has_view) {
                    $view = view('backend.datatable.action-view')
                        ->with(['route' => route('admin.lessons.show', ['lesson' => $q->id]), 'seq' => $sequance])->render();
                }
                if ($has_edit) {
    ...
    function reorderLesson(Request $request){

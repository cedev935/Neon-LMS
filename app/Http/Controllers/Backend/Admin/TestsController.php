...
class TestsController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if (isset($request->course_id) && $request->course_id != "0") {
            ...
            $tests = Test::query()->orderBy('testorders', 'asc');
        }
        ...
        return DataTables::of($tests)
            ...
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $request) {
                $sequance = $q->courseTimeLine['sequence']??null;
                $view = "";
                $edit = "";
                $delete = "";
                if ($request->show_deleted == 1) {
                ...
                if ($has_view) {
                    $view = view('backend.datatable.action-view')
                        ->with(['route' => route('admin.tests.show', ['test' => $q->id]), 'seq' => $sequance])->render();
                }
                if ($has_edit) {
    ...
    {   
        ...
        $qus['color1'] = $request->color1;
        $qus['color2'] = $request->color2;


        Question::where('test_id', 'like', '%'.$id.'%')->update($qus);
    ...
    function order_edit(Request $request){
        ...
        //$ids = $request->id_info;
        $test_id = json_decode($request->test_id);
        $sequences = json_decode($request->sequence);
        $courseid = $request->courseid;

        $timeline = CourseTimeline::where('model_type', '=', Test::class)
                    ->whereIn('model_id', $data)
                    ->where('course_id', $courseid)->get();
        for ($i = 0; $i<count($timeline); $i++){
                $timeline[$i]->sequence = $sequences[$i];
                $timeline[$i]->save();             
        }

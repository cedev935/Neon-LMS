...
class ChartsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend.charts.index', compact('courses', 'tests'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        if (isset($request->test_id) && $request->test_id != "") {
            if (isset($request->course_id) && $request->course_id != "") {
                ...
                ->whereIn('chart_test.test_id', $test_ids)
    ...
    public function create(Request $request)
    {
        ...
        $tests =DB::table('tests')->select('title','id')->whereIn('course_id', $courses_ids)->get();
        ...
        else
        {
            ...
            for ($i=0;$i <count($test_question_list);$i++)
            {
                $temp =DB::table('questions')
                    ...
                    ->select('id','question','questiontype','logic', 'content', 'score','questions.test_id as qtest_id')
            ...
            foreach($file_questions as $key => $value){
                if (isset($value->content)) {
                    $decodedContent = json_decode($value->content);
            
                    if ($decodedContent !== null && is_array($decodedContent) && isset($decodedContent[0]->number)) {
                        $file_num = $decodedContent[0]->number;
    ...
    public function edit($id)
    {
        ...
        $tests = DB::table('tests')->whereIn('course_id', $courses_ids)->select('title','id')->get();
        ...
        else
        {
            ...
            foreach($file_questions as $key => $value){
                if(is_array($value->content) && isset($value->content)){
                    $file_num = json_decode($value->content)[0]->number;
                    $file_questions[$key]->file_num = $file_num; 
                }else{
                    $file_questions[$key]->file_num = 0;
                }
            }
            $file_questions = json_decode(json_encode($file_questions), true);

            return view('backend.charts.edit')
                ->with('courses', $courses)
                ->with('chart',$chart)->with('current_tests',$current_tests)->with('tests', $tests)->with('course_list',$course_list)

    ...
    jQuery(document).ready(function (e) {
        ...
        @else
            @if(request('course_id') != "")
                route = '{{route('admin.charts.get_data',['course_id' => request('course_id')])}}';
            @endif
        @endif
        ...
        $(document).on('change', '#test_id', function (e) {
            ...
            window.location.href = "{{route('admin.charts.index')}}" + "?course_id=" + course_id + "&test_id=" + test_id;
        });
        $(document).on('change', '#course_id', function (e) {
            var course_id = $(this).val();
            window.location.href = "{{route('admin.charts.index')}}" + "?course_id=" + course_id;
        });
        @can('question_delete')
        @if(request('show_deleted') != 1)

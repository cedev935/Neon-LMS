...
@section('title', __('Text Groups').' | '.app_name())
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-right">
                ...
                <a href="{{ route('admin.textgroups.create', [
        ...
        <div class="card-body table-responsive">
            ...
            <div class="d-block">
                <ul class="list-inline">
                    <li class="list-inline-item"><a href="{{ route('admin.textgroups.index') }}"
                                                    style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">{{trans('labels.general.all')}}</a>
                    </li>
                    |
                    <li class="list-inline-item"><a href="{{ route('admin.textgroups.index') }}?show_deleted=1"
...
jQuery(document).ready(function (e) {
    var route = "{{route('admin.textgroups.get_data')}}";

    {{--  @if(request('show_deleted') == 1)
        route = '{{route('admin.textgroups.get_data',['show_deleted' => 1])}}';
    @endif  --}}

    @if(request('test_id') != "")
        route = '{{route('admin.textgroups.get_data',['test_id' => request('test_id')])}}';
    @else
        @if(request('course_id') != "")
            route = '{{route('admin.textgroups.get_data',['course_id' => request('course_id')])}}';
        @endif
    @endif
    ...
    $(document).on('change', '#course_id', function (e) {
        ...
        window.location.href = "{{route('admin.textgroups.index')}}" + "?course_id=" + course_id;
    });

    @can('textgroup_delete')
    @if(request('show_deleted') != 1)
    $('.actions').html('<a href="' + '{{ route('admin.textgroups.mass_destroy') }}' + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Delete selected</a>');
    @endif
    @endcan

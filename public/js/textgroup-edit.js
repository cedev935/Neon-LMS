...
var init_treeview = function (text_id, condition_id) {
    $('#condition_'+text_id+'_'+condition_id).find('.logic_tree').jstree({
        "core" : {
            "themes" : {
                "responsive": false
            }            
        ...
        if (name.length>1) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var html_append = ``;
                    qt_type_in.val(type);
                    qt_nm.val(response[0]['id'] + "." + response[0]['question'].replace(/<(.|\n)*?>/g, ''));
                    if (type == 1) {
                        ...
                        for (var i = 0; i < content.length - 2; i++) {
                            ...
                            html_append += `<div class="checkbox"><label>
                                                <input class="checkbox_` + i + ` check_box_q" name="radio_` + response[0]['id'] + `_` + text_id + `" type="checkbox" value="` + score + `" ` + is_checked + ` >` + label + `
                                            </label></div>`;
                        }
                    ...
                    html_append += `            </div>
                        ...
                        html_append = `<div class="row main-content" id="cond_` + response[0]['id'] + `"  >
                                        ...
                                        <div class="col-4">
                                            <div class="form-body">                                    
                                                <div class="form-group ">`;
                                            if (response[0]['questionimage']!=null && response[0]['questionimage']!='')    
                        html_append += `            <img class="display-image-preview" src="/uploads/image/` + response[0]['questionimage'] + `" style="max-height: 150px;" />`;
                        html_append += `     </div>
...
var clone_condition = function( text_id, condition_id ) {
    var t_c = $('#text_'+text_id).children('.condition-box').children('.condition').last().attr('id');
    var new_condition_id = (t_c==null) ? 0 : parseInt(t_c.split('_')[2])+1;
    var in_html = $('#condition_'+text_id+'_'+condition_id).html();
    in_html = in_html.replaceAll('condition('+text_id+', '+condition_id+')', 'condition('+text_id+', '+new_condition_id+')');
    var new_html = '<div class="row mt-3 condition" id="condition_'+text_id+'_'+new_condition_id+'">'+ in_html + '</div>';
    $('#text_'+text_id).children('.condition-box').append(new_html);
    $('#condition_'+text_id+'_'+new_condition_id+' .qt_name').val($('#condition_'+text_id+'_'+condition_id+' .qt_name').val());
    $( document ).ready(function() {

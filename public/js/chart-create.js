...
// Object to store the current values of inputs
var savedInputValues = {};

// Function to save the values of all inputs in the matrix
function saveMatrixValues() {
    $('#real_matrix').find('input[type="text"]').each(function() {
        var id = $(this).attr('id');
        if(id) {
            savedInputValues[id] = $(this).val();
        }
    });
    console.log("savedInputValues:",savedInputValues)
}

// Function to restore the values to inputs after matrix rebuild
function restoreMatrixValues() {
    for (var id in savedInputValues) {
        $('#' + id).val(savedInputValues[id]);
    }
}
...
// saveMatrixValues();
        ...
        for (var j=2;j<= $("#row_panel").children().length ; j++)
        {
            ...
            for (var i=2;i<= $("#col_panel").children().length ; i++)
            {
                ...
                html_cont += `<td> <input type="text"  placeholder="" class="clickable-input form-control formulas" id="`+idx+`" onfocus="selectItemFunction(event)"></td>`;
            }
            html_cont +=`</tr>`;
        }
...
window.configTree = (tree_name) => {
    ...
    $(`.${tree_name}`).on('select_node.jstree', function(e, data) { 
        e.preventDefault();
        var radio_id = 0;
...
jQuery(document).ready(function(e) {   
    $(document).on('click', '.qt_name', function (e) {
        const tree_name = $(e.target).data('tree');
        $(this).siblings(`.${tree_name}`).toggle();
    });

    // document.addEventListener('click', event => {
    //     if (
    //         !$(event.target).hasClass('qt_name')
    ...
    configTree('question_tree');
    configTree('textgroup_tree');
});